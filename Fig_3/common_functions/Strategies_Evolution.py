...
def Evolution_Braitenberg(
    n, b, t, AntennaeLength, AgentPosition, FlowGradient, e3, dt, V, twist
):
    ...
    # Rotation around B
    AngularSpeed = Braitenberg_Control(
        GradientIntensity_s1, GradientIntensity_s2, AntennaeLength, b, dt
    )
...
def Evolution_Triangulation(
    ...
    # Detected information
    d0 = np.dot(FlowGradient(e3, AgentPosition), n)
    d1 = np.dot(FlowGradient(e3, xs1), n)
    d2 = np.dot(FlowGradient(e3, xs2), n)

    # Triangulation inferred direction d
    # TargetDirection = Triangulation3D(xs1, d1, xs2, d2)
    TargetDirection = Triangulation3DH(n, d0, d2 - d1)
...
def Evolution_Intensity_MS(
    n, b, t, AntennaeLength, BodyWidth, AgentPosition, FlowGradient, e3, dt, V, twist=0
):
    ...
    # Rotation around n
    AngularSpeed = Braitenberg_Control(
        GradientIntensity_s1_2, GradientIntensity_s2_2, AntennaeLength, -new_n, dt
    )
    Norm = np.linalg.norm(AngularSpeed)
