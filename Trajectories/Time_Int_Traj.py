...
from Geometric_functions import * # Import Rotation
...
def Evolution(Grad, Strategy, twist):
	...
	Thetas = np.linspace(0.2, np.pi/2, Nagents)
	...
	for a in range(Nagents):
		...
		while np.linalg.norm(AgentPosition) > RMIN and time < TMAX:
			time += 1
			n, b, t, AgentPosition = Strategy(n, b, t, AntennaeLength, AgentPosition, Grad, e3, dt, V, twist)
