...
def Evolution(Grad, Strategy, twist):
	...
	TMAX = 20*np.pi*(R0/V) # Max time allowed in simulations
	dt = 0.1 # Timesteps 
	...
	for k in range(NTrials):
		...
		while np.linalg.norm(AgentPosition) > RMIN and time < TMAX:
			...
			n, b, t, AgentPosition = Strategy(n, b, t, AntennaeLength, AgentPosition, Grad, e3, dt, V, twist, 0) # Add BodyWidth for 4 sensors strategy
