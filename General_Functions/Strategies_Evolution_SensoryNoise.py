# Strategies_Evolution_MeasureNoise - Contain strategies implementation: 
# - Evolution_Braitenberg : Apply Braitenberg strategy
# - Evolution_Triangulation : Apply Triangulation strategy
# both with Measure noise implemented
...
def Evolution_Braitenberg(n, b, t, AntennaeLength, AgentPosition, FlowGradient, e3, dt, twist, Intensity):
    ...
    # Generate noise
    MeasureNoise_s1 = np.random.normal(size=3, scale=1)*np.sqrt(Intensity)
    MeasureNoise_s2 = np.random.normal(size=3, scale=1)*np.sqrt(Intensity)

    # Detected information
    GradientIntensity_s1 = np.linalg.norm(np.dot(FlowGradient(e3, xs1), n)  + MeasureNoise_s1)
    GradientIntensity_s2 = np.linalg.norm(np.dot(FlowGradient(e3, xs2), n)  + MeasureNoise_s2)
...
def Evolution_Intensity_MS(n, b, t, AntennaeLength, BodyWidth, AgentPosition, FlowGradient, e3, dt, twist, Intensity):
    ...
    # Detected information
    GradientIntensity_s1 = np.linalg.norm(np.dot(FlowGradient(e3, xs1_n), n)  + MeasureNoise_s1)
    GradientIntensity_s2 = np.linalg.norm(np.dot(FlowGradient(e3, xs2_n), n)  + MeasureNoise_s2)
    ...
    # Detected information
    GradientIntensity_s1_2 = np.linalg.norm(np.dot(FlowGradient(e3, xs1_b), b)  + MeasureNoise_s1)
    GradientIntensity_s2_2 = np.linalg.norm(np.dot(FlowGradient(e3, xs2_b), b)  + MeasureNoise_s2)
...
def Evolution_Triangulation_HardNoise(n, b, t, AntennaeLength, AgentPosition, FlowGradient, e3, dt, twist, Intensity):
    ...
    # Generate noise
    MeasureNoise_s0 = np.random.normal(size=3, scale=1)*np.sqrt(Intensity)
    MeasureNoise_s1 = np.random.normal(size=3, scale=1)*np.sqrt(Intensity)
    MeasureNoise_s2 = np.random.normal(size=3, scale=1)*np.sqrt(Intensity)

    # Detected information
    DirectionalGradient_s0 = np.dot(FlowGradient(e3, AgentPosition), n) + MeasureNoise_s0
    DirectionalGradient_s1 = np.dot(FlowGradient(e3, xs1), n) + MeasureNoise_s1
    DirectionalGradient_s2 = np.dot(FlowGradient(e3, xs2), n) + MeasureNoise_s2
...
def Evolution_Triangulation(n, b, t, AntennaeLength, AgentPosition, FlowGradient, e3, dt, twist, Intensity):
    ...
    # Generate noise
    MeasureNoise_s0 = np.random.normal(size=3, scale=1)
    ...
    AngularSpeed /= np.linalg.norm(AngularSpeed)
    Angle = np.clip(np.dot(t, TargetDirection), a_min = -1, a_max = 1)
    Norm = np.arccos(Angle)*dt + Intensity*np.sqrt(dt)
...
def Evolution_Triangulation_NoiseSign(n, b, t, AntennaeLength, AgentPosition, FlowGradient, e3, dt, twist, Intensity):
    ...
    # Triangulation inferred direction d
    TargetDirection = Triangulation3DHNoise(n, DirectionalGradient_s0, DirectionalGradient_s2-DirectionalGradient_s1, Intensity)
